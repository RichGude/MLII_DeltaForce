...
inputShape = (IMG_HEIGHT, IMG_WIDTH, IMG_CHAN)

# define a left and right model in order to weight the two halves separately =>
#    then bring them together for a final output

# Define the inputs to either side
inputL = Input(shape=inputShape)
inputR = Input(shape=inputShape)
...
leftMdl = Conv2D(36, (5, 5),            # output channels = 36, and kernel size = 5x5 shrinking the dimensions
                 strides=(2, 2),
                 padding='valid',       # input shape does NOT have the same shape as the output
                 activation='elu',
                 kernel_initializer='he_normal')(leftMdl)
leftMdl = Dropout(0.2)(leftMdl)
leftMdl = Conv2D(48, (5, 5),            # output channels = 48, and kernel size = 5x5 shrinking the dimensions
                 strides=(2, 2),
                 padding='valid',       # input shape does NOT have the same shape as the output
                 activation='elu',
                 kernel_initializer='he_normal')(leftMdl)
leftMdl = Dropout(0.1)(leftMdl)
leftMdl = Conv2D(64, (3, 3),            # output channels = 64, and kernel size = 3x3 shrinking the dimensions
                 strides=(2, 2),
                 padding='valid',       # input shape does NOT have the same shape as the output
                 activation='elu',
                 kernel_initializer='he_normal')(leftMdl)
leftMdl = Flatten()(leftMdl)
leftMdl = Dense(100, activation='elu', kernel_initializer='he_normal')(leftMdl)
leftMdl = Dense(50, activation='elu', kernel_initializer='he_normal')(leftMdl)
leftMdl = Dropout(0.1)(leftMdl)
leftMdl = Dense(10, activation='elu', kernel_initializer='he_normal')(leftMdl)
# Do not put activation at the end because we want to exact output, not a class identifier
leftMdl = Dense(1, kernel_initializer='he_normal')(leftMdl)
leftMdl = Model(inputs=inputL, outputs=leftMdl)

# Define right model
rghtMdl = Lambda(lambda x: x / 127.5 - 1)(inputR)       # perform custom normalization before lambda layer in network
# Three layers of 2D convolutional layers
rghtMdl = Conv2D(24, (5, 5),            # output channels = 24, and kernel size = 5x5 shrinking the dimensions
                 strides=(2, 2),
                 padding='valid',       # input shape does NOT have the same shape as the output
                 activation='elu',      # use ELU, vice ReLu, for dense-flow
                 kernel_initializer='he_normal')(rghtMdl)
rghtMdl = Conv2D(36, (5, 5),            # output channels = 36, and kernel size = 5x5 shrinking the dimensions
                 strides=(2, 2),
                 padding='valid',       # input shape does NOT have the same shape as the output
                 activation='elu',
                 kernel_initializer='he_normal')(rghtMdl)
rghtMdl = Dropout(0.2)(rghtMdl)
rghtMdl = Conv2D(48, (5, 5),            # output channels = 48, and kernel size = 5x5 shrinking the dimensions
                 strides=(2, 2),
                 padding='valid',       # input shape does NOT have the same shape as the output
                 activation='elu',
                 kernel_initializer='he_normal')(rghtMdl)
rghtMdl = Dropout(0.1)(rghtMdl)
rghtMdl = Conv2D(64, (3, 3),            # output channels = 64, and kernel size = 3x3 shrinking the dimensions
                 strides=(2, 2),
                 padding='valid',       # input shape does NOT have the same shape as the output
                 activation='elu',
                 kernel_initializer='he_normal')(rghtMdl)
rghtMdl = Flatten()(rghtMdl)
rghtMdl = Dense(100, activation='elu', kernel_initializer='he_normal')(rghtMdl)
rghtMdl = Dense(50, activation='elu', kernel_initializer='he_normal')(rghtMdl)
rghtMdl = Dropout(0.1)(rghtMdl)
rghtMdl = Dense(10, activation='elu', kernel_initializer='he_normal')(rghtMdl)
# Do not put activation at the end because we want to exact output, not a class identifier
rghtMdl = Dense(1, kernel_initializer='he_normal')(rghtMdl)
rghtMdl = Model(inputs=inputR, outputs=rghtMdl)

# Define a combined model taking the maximum speed recorded from each side of the model
combMdl = Maximum()([leftMdl.output, rghtMdl.output])
model = Model(inputs=[leftMdl.input, rghtMdl.input], outputs=combMdl)

# Define the optimizer and compile the model
opt = Adam(lr=1e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.001)
